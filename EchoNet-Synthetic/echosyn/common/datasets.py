...
class Dynamic(Dataset):
    def __init__( self, config, split=["TRAIN", "VAL", "TEST"], datafolder="Videos", ext=".avi") -> None:
        ...
        assert [o in ["video", "image", "lvef"] for o in self.outputs], \
        ...
        self.duration_seconds = (self.duration_frames / self.target_fps
                                 if type(self.target_fps) == int else None)
    ...
    def __getitem__(self, idx, return_row=False):
        ...
        output = {"filename": row["FileName"], "still": False,}

        if "image" in self.outputs or "video" in self.outputs:
            reader = decord.VideoReader(row["VideoPath"], ctx=decord.cpu(),
        ...
        if "video" in self.outputs:
            ...
            start_idx = (np.random.choice(np.arange(0, resample_indices_a[1]))
                         if len(resample_indices_a) > 1 and resample_indices_a[1] > 1 else 0)
            resample_indices_a = resample_indices_a + start_idx

            # Sample a random chunk to cover the requested duration
            start_idx = (np.random.choice(np.arange(0, len(resample_indices_a) - self.duration_frames))
                         if len(resample_indices_a) > self.duration_frames else 0)
            ...
            if self.target_fps == "exponential":
                ...
                output["indices"] = np.concatenate((resample_indices_b,
                                                    np.repeat(resample_indices_b[-1], self.duration_frames - len(resample_indices_b),),))
...
class Latent(Dynamic):
    def __init__(self, config, split=["TRAIN", "VAL", "TEST"]) -> None:
        ...
        if config.get("motion", False):
            logging.info(f'Load motion features from path {config.motion}.')
            self.motion_folder = config.motion
            self.motion_files = {}
            for s in split:
                motion_files = os.listdir(os.path.join(self.motion_folder, s))
                self.motion_files[s] = motion_files

        remove_idx = []
        for i, path in enumerate(self.metadata['FileName']):
            base_path = path + '.pt'
            if base_path not in self.motion_files[split[0]]:
                remove_idx.append(i)
        if len(remove_idx) > 0:
            self.metadata.drop(remove_idx, inplace=True)
            self.metadata.reset_index()

    def _get_motion_file(self, fname_no_ext):
        fname = fname_no_ext + ".pt"
        for s in self.motion_files.keys():
            if fname in self.motion_files[s]:
                return os.path.join(self.motion_folder, s, fname)
    ...
    def __getitem__(self, idx, return_row=False):
        ...
        if "video" in self.outputs:
            ...
            start_idx = (np.random.choice(np.arange(0, resample_indices[1]))
                         if len(resample_indices) > 1 and resample_indices[1] > 1 else 0)
            resample_indices = resample_indices + start_idx

            # Sample a random chunk to cover the requested duration
            start_idx = (np.random.choice(np.arange(0, len(resample_indices) - self.duration_frames))
        ...
        if "motion" in self.outputs:
            motion_file = self._get_motion_file(row["FileName"].split(".")[0])
            # try:
            motion_tensor = torch.load(motion_file, map_location="cpu")
            # except:
            #     motion_tensor = torch.zeros((3072,))
            #     print(f"Error loading motion file {motion_file}")
            output["motion"] = motion_tensor
...
class RandomVideo(Dataset):
    def __init__(self, config, split=["TRAIN", "VAL", "TEST"]) -> None:
        ...
        assert all([o in ["video", "image"] for o in self.outputs]
    ...
    def __getitem__(self, idx):
        ...
        if "video" in self.outputs:
            if (self.still_image_p > torch.rand(1,).item()):
                ...
                paths = [os.path.join(self.root, self.all_frames[ridx])
...
class TensorMotionEFSpecial(Dataset):
    def __init__(self, root, motion, split=None):
        self.tensor_root = root
        self.motion_root = os.path.join(motion, 'TRAIN')

        print('generate videos from EF 90 and largest motion.')

        # df = pd.read_csv(
        #     "/vol/ideadata/at70emic/projects/EchoSynExt/datasets/EchoNet-Dynamic/FileList.csv"
        # )

        # df_filt1 = df[(df["EF"] > 70) & (df["Split"] == "TRAIN")]
        # df_filt2 = df[(df["EF"] < 30) & (df["Split"] == "TRAIN")]
        #
        # motions = df_filt1["FileName"].tolist() + df_filt2["FileName"].tolist()

        self.all_tensors = glob(os.path.join(self.tensor_root, "*.pt"))
        # self.all_motions = glob(os.path.join(self.motion_root, "*.pt"))
        # self.key = [31, 38]

        # self.all_motions = [t for t in self.all_motions if os.path.basename(t).split(".")[0] in motions]
        # self.all_tensors = sorted(self.all_tensors)

        # split_size = len(self.all_tensors) // 3
        # start = split * split_size
        # end = (split + 1) * split_size

        # self.all_tensors = self.all_tensors[start:end]
        self.motion = torch.load(
            "/vol/idea_longterm/ot70igyn/EMA-VFI/log/train_EchoNet/20250326-174659/real_train44/TRAIN/0X354B37A25C64276F.pt",
            map_location="cpu",)
    ...
    def __getitem__(self, idx):
        tensor_idx = idx % len(self.all_tensors)
        # motion_idx = idx % len(self.all_motions)
        tensor = torch.load(self.all_tensors[tensor_idx], map_location="cpu")
        frame_idx = np.random.randint(0, tensor.size(0), 1)[0]
        tensor = tensor[frame_idx]
        # motion = torch.load(self.all_motions[motion_idx], map_location="cpu")
        lvef = 90
        return tensor, self.motion, lvef
...
class TensorMotionEF(Dataset):
    def __init__(self, root, motion, split=None):
        self.tensor_root = root
        self.motion_root = os.path.join(motion, 'TRAIN')

        print('generate videos from all EF and motion features.')

        df = pd.read_csv(
            "/vol/idea_longterm/ot70igyn/EchoNet-Synthetic/data/EchoNet-Dynamic/FileList.csv")
        ...
        self.all_tensors = glob(os.path.join(self.tensor_root, "*.pt"))
        self.all_motions = glob(os.path.join(self.motion_root, "*.pt"))
        motion_files = os.listdir(self.motion_root)

        temp = []
        for path in self.all_tensors:
            t = os.path.basename(path)
            if t in motion_files:
                temp.append(path)

        self.all_tensors = temp

        self.all_tensors = sorted(self.all_tensors)
        self.all_motions = sorted(self.all_motions)
        self.motion_files = sorted(motion_files)

        self.frames = collections.defaultdict(list)
        self.trace = collections.defaultdict(_defaultdict_of_lists)
        with open('/vol/idea_longterm/ot70igyn/EchoNet-Synthetic/data/EchoNet-Dynamic/VolumeTracings.csv') as f:
        ...
        for m in self.motion_files:
            m = m.split(".")[0]
            inx_ef = df_train[df_train['FileName'] == m].index.tolist()[0]
            self.lvef_train.append(df_train.loc[inx_ef]['EF'])

            inx_frame = self.frames[m+'.avi']
            self.large_frame.append(inx_frame[-1])

        # save EF and large number
    ...
    def __getitem__(self, idx):
        tensor_idx = idx % len(self.all_tensors)
        motion_idx = idx % len(self.all_motions)
        ef_idx = idx % len(self.lvef_train)
        # frame_idx = idx % len(self.large_frame)

        tensor_name = self.all_tensors[tensor_idx]
        tensor = torch.load(tensor_name, map_location="cpu")
        frame_idx = np.random.randint(0, tensor.size(0), 1)[0]
        # large_idx = self.large_frame[frame_idx]
        tensor = tensor[frame_idx]

        motion_name = self.all_motions[motion_idx]
        motion = torch.load(motion_name, map_location="cpu", weights_only=True)
        lvef = self.lvef_train[ef_idx]
        return tensor, motion, lvef, tensor_name, motion_name


class TensorMotionSpecial(Dataset):
    def __init__(self, root, motion, split=None):
        self.tensor_root = root
        self.motion_root = os.path.join(motion, 'TRAIN')
        print('generate videos from only largest motion.')
        ...
        # self.all_tensors = self.all_tensors[start:end]
        self.motion = torch.load(
            "/vol/idea_longterm/ot70igyn/EMA-VFI/log/train_EchoNet/20250804-065650/motion_74/TRAIN/0X354B37A25C64276F.pt",
            map_location="cpu",)
    ...
    def __getitem__(self, idx):
        ...
        tensor = tensor[frame_idx]
        # motion = torch.load(self.all_motions[motion_idx], map_location="cpu")
        return tensor, self.motion


class TensorMotionSet(Dataset):
    def __init__(self, root, motion, split=0):
        self.tensor_root = root
        self.motion_root = os.path.join(motion, 'TRAIN')
        print(f'Load motion features from {root}.')
        self.split = split
    ...
    def __getitem__(self, idx):
        tensor_idx = idx % len(self.all_tensors) + self.split
        motion_idx = idx % len(self.all_motions) + self.split
        tensor_name = self.all_tensors[tensor_idx]
        ...
        # frame_idx = idx % len(self.large_frame)
        # large_idx = self.large_frame[frame_idx]
        # tensor = tensor[large_idx]

        motion_name = self.all_motions[motion_idx]
        motion = torch.load(motion_name, map_location="cpu", weights_only=True)
        return tensor, motion, tensor_name, motion_name
