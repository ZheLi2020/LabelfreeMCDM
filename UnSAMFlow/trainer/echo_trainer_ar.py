...
class TrainFramework(BaseTrainer):
    def __init__(self, train_loader, valid_loader, model, loss_func, save_root, config, data_name,
                 resume=False, train_sets_epoches=None, rank=0, world_size=1, device=None, n_gpu=None):
        super(TrainFramework, self).__init__(train_loader, valid_loader, model, loss_func, save_root,
                                             config, data_name, resume=resume, train_sets_epoches=train_sets_epoches,
                                             rank=rank, world_size=world_size, device=device, n_gpu=n_gpu)
    ...
    def _run_one_epoch(self):
        ...
        timing_meter_names = ["1_data_loading", "2_main_forward", "3_main_loss",
                              "4_atst", "5_ot", "6_bakward_update",]
        ...
        while i_step < self.cfg.epoch_size:
            for data in self.train_loaders[self.i_train_set]:
                ...
                flows = [torch.cat([flow12, flow21], 1) for flow12, flow21 in zip(flows_12, flows_21)]

                (loss, l_ph, l_sm, flow_mean, flow_vis_mask12, flow_vis_mask21,) = self.loss_func(
                ...
                if self.cfg.run_atst:
                    ...
                    s = {"imgs": [img1, img2], "full_segs": [full_seg1, full_seg2],
                         "flows_f": [flow_ori], "masks_f": [noc_ori],}
                    ...
                    flow_t_pred = self.model(img1_st, img2_st, full_seg1_st, full_seg2_st, with_bk=False)["flows_12"][0]
                ...
                if self.cfg.run_ot:
                    ...
                    img1_ot, img2_ot = data["img1_ph"].to(self.device), data["img2_ph"].to(self.device)
                    ...
                    if (self.cfg.key_obj_aug
                        ...
                        B = img1_ot.shape[0]
                        out = self.obj_cache.pop(B * self.cfg.key_obj_count, with_aug=True)
                        ...
                        for _round in range(self.cfg.key_obj_count):
                            ...
                            (img1_ot, img2_ot, full_seg1_ot, full_seg2_ot, flow_ot, noc_ot, obj_mask2,) = output

                    # run 3rd pass
                    imgs = torch.cat([img1_ot, img2_ot], 1)
                    full_segs_ot = torch.cat([full_seg1_ot, full_seg2_ot], 1)

                    imgs, full_segs_ot, flow_ot, noc_ot = random_crop(imgs, full_segs_ot, flow_ot, noc_ot, self.cfg.ot_size)
                    img1_ot, img2_ot = imgs[:, :3], imgs[:, 3:6]
                    full_seg1_ot, full_seg2_ot = (full_segs_ot[:, :1], full_segs_ot[:, 1:],)
                    # adj_map1_ot, adj_map2_ot = full_segs_to_adj_maps(
                    #     full_seg1_ot
                    # ), full_segs_to_adj_maps(full_seg2_ot)

                    res_dict_ot = self.model(img1_ot, img2_ot, full_seg1_ot, full_seg2_ot, with_bk=False)
                ...
                # update meters
                key_meters.update([loss.item(), l_ph.item(), l_sm.item(), l_atst.item(),
                                   l_ot.item(), flow_mean.item(),], img1.shape[0],)
                ...
                if (self.i_iter + 1) % self.cfg.record_freq == 0:
                    ...
                    if self.rank == 0:
                        for v, name in zip(key_meters_vals, key_meter_names):
                            wandb.log({"train:{}/".format(self.data_name) + name: v}, step=self.i_iter + 1)
                        wandb.log({"train:{}/learning_rate".format(self.data_name): self.optimizer.param_groups[0]["lr"]}, step=self.i_iter + 1)

                        for v, name in zip(timing_meters.avg, timing_meter_names):
                            wandb.log({"timing_batch_avg/" + name: v}, step=self.i_iter + 1)

                        timing_meters.reset(i=len(timing_meter_names))

                if (self.i_iter + 1) % self.cfg.print_freq == 0:
                    if self.rank == 0:
                        istr = "{}:{:04d}/{:04d}".format(self.i_epoch, i_step + 1, self.cfg.epoch_size)\
    ...
    def _run_one_epoch_echo(self):
        ...
        while i_step < self.cfg.epoch_size:
            for data in self.train_loaders:
                ...
                # read data to device
                img1, img2 = data["img0"].to(self.device), data["img1"].to(self.device)
                full_seg1, full_seg2 = data["img0_segs"].to(self.device), data["img1_segs"].to(self.device)
                ...
                loss, l_ph, l_sm, flow_mean, flow_vis_mask12, flow_vis_mask21 = self.loss_func(
                    flows, img1, img2, occ_aware=False, full_seg1=full_seg1, full_seg2=full_seg2)
                loss = loss.mean()
                l_ph = l_ph.mean()
                l_sm = l_sm.mean()
                flow_mean = flow_mean.mean()
                ...
                ## ARFlow appearance/spatial transform
                if self.cfg.run_atst:
                    img1, img2 = data["img0_ph"].to(self.device), data["img1_ph"].to(self.device)
                    s = {"imgs": [img1, img2], "full_segs": [full_seg1, full_seg2],
                         "flows_f": [flow_ori], "masks_f": [noc_ori],}
                ...
                if self.cfg.run_ot:
                    ...
                    img1_ot, img2_ot = data["img0_ph"].to(self.device), data["img1_ph"].to(self.device)
                    full_seg1_ot, full_seg2_ot = data["img0_segs"].to(self.device), data["img1_segs"].to(self.device)
                    ...
                    # push current object mask into cache for future use
                    if self.cfg.key_obj_aug:
                        valid_idx = ~torch.isnan(data["img0_obj"][:, 0, 0, 0])

                        if valid_idx.sum() > 0:  # at least one valid object
                            obj_mask = data["img0_obj"][valid_idx]
                            img = data["img0_ph"][valid_idx]
                            mean_flow = (flow_ori[valid_idx].cpu() * obj_mask).mean(dim=[2, 3]) / obj_mask.mean(dim=[2, 3])
            ...
            for v, name in zip(key_meters.val, key_meter_names):
                wandb.log({"train:{}/".format(self.data_name) + name: v}, step=self.i_epoch + 1)
            wandb.log({"train:{}/learning_rate".format(self.data_name): self.optimizer.param_groups[0]["lr"]},
                      step=self.i_epoch + 1)

            for v, name in zip(timing_meters.avg, timing_meter_names):
                wandb.log({"timing_batch_avg/" + name: v}, step=self.i_epoch + 1)

            if key_meters.val[0] < self.best_loss:
                self.best_loss = key_meters.val[0]
                self.save_model(name="model_best{}".format(self.i_epoch))
    ...
    def _validate_with_gt(self):
        ...
        for i_set, loader in enumerate(self.valid_loaders):
            ...
            for i_step, data in enumerate(loader):
                ...
                res = [load_flow(p) for p in data["flow_occ"]]
                gt_flows, occ_masks = [r[0] for r in res], [r[1] for r in res]

                # res = list(map(load_flow, data["flow_noc"]))
                res = [load_flow(p) for p in data["flow_noc"]]
                noc_masks = [r[1] for r in res]
                gt_flows = [np.concatenate([flow, occ_mask, noc_mask], axis=2)
            ...
            for value, name in zip(flow_error_meters.avg, flow_error_names):
                wandb.log({"valid{}:{}_flow/".format(i_set, name_dataset) + name: value}, step=self.i_iter)
    ...
    def validate_without_gt(self, cfg):
        ...
        # test = '0X25624BE57690EB19.avi'
        # if test in self.train_loaders.dataset.fnames:
        #     print(test)
        # if test in self.valid_loaders.dataset.fnames:
        #     print(test)

        for i_set, loader in enumerate([self.train_loaders, self.valid_loaders]):
            idx = 0
            flow_error_names = ["EPE_all", "EPE_noc", "EPE_occ", "Fl_all", "Fl_noc"]
            flow_error_meters = AverageMeter(i=len(flow_error_names))

            for i_step, data in enumerate(tqdm(loader)):
                # compute output
                img1 = data["img0"].to(self.device)
                img2 = data["img1"].to(self.device)
                middle = data["middle"].to(self.device)
                full_seg1, full_seg2 = data["img0_segs"].to(self.device), data["img1_segs"].to(self.device)
                middle_seg = data["middle_segs"].to(self.device)

                img1_all = torch.cat([middle, img1], dim=0)
                img2_all = torch.cat([middle, img2], dim=0)
                full_seg1_all = torch.cat([middle_seg, full_seg1], dim=0)
                full_seg2_all = torch.cat([middle_seg, full_seg2], dim=0)

                res_dict = self.model(img1_all, img2_all, full_seg1_all, full_seg2_all, with_bk=True)
                flows = res_dict["flows_12"]
                pred_flows = flows[0].detach().cpu().numpy().transpose([0, 2, 3, 1])

                save_path = os.path.join(cfg.save_root, 'pred_flow')
                # os.makedirs(save_path, exist_ok=True)
                flow_name = data['video_path'][0]
                flow_name = os.path.basename(flow_name).split('.')[0]
                flow_npy = flow_name + '.npy'
                np.save(os.path.join(save_path, flow_npy), pred_flows)

                if idx < 101:
                    for i in range(pred_flows.shape[0]):
                        save_img = os.path.join(cfg.save_root, 'pred_flow_img')
                        flow_png = flow_name + f'_{i}.png'

                        mag, ang = cv2.cartToPolar(pred_flows[i][..., 0], pred_flows[i][..., 1])
                        ...
                        imageio.imwrite(os.path.join(save_img, flow_png), flow_img.astype(np.uint8))
                else:
                    break
