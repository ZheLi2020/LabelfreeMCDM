...
def load_model(path):
    ...
    assert getattr(klass, "from_pretrained",
                   None) is not None, f"Class {klass_name} does not support 'from_pretrained'."
...
def save_as_avi(tensor, filename, fps=30):
    ...
    # buffer = io.BytesIO()
    iio.imwrite(filename, np_video, format_hint=".avi", plugin="pyav", fps=fps, codec="h264",)
