...
class EchoDynamicOne(torchvision.datasets.VisionDataset):
    """EchoNet-Dynamic Dataset.
    ...
    def __init__(self, root=None, input_transform=None, co_transform=None, ap_transform=None,
                 seg_root=None, obj_root=None, split="train", target_type="EF",
                 length=32, period=2, max_length=250, ):
        if root is None:
    ...
    def get_frames(self, index):
        ...
        small_frames = np.transpose(small_frames, (1, 2, 0))
        large_frames = np.transpose(large_frames, (1, 2, 0))

        return small_frames, large_frames, video_path

    def get_seg(self, video_name):
        seg0_path = os.path.join(self.seg_root, video_name.split('.')[0] + '_img0.png')
        seg0 = imageio.imread(seg0_path)

        seg1_path = os.path.join(self.seg_root, video_name.split('.')[0] + '_img1.png')
        seg1 = imageio.imread(seg1_path)

        # return img0_segs, img1_segs
        return seg0, seg1

    def get_obj(self, video_name):
        obj0_name = os.path.join(self.obj_root, video_name.split('.')[0] + '_img0.npy')
        key_objs0 = (np.load(obj0_name) / 255.0)

        obj1_name = os.path.join(self.obj_root, video_name.split('.')[0] + '_img1.npy')
        key_objs1 = (np.load(obj1_name) / 255.0)

        # return img0_obj, img1_obj
        return key_objs0, key_objs1

    def __getitem__(self, index):
        img0, img1, video_path = self.get_frames(index)
        vid_name = os.path.basename(video_path).split('.')[0]
        img0_segs, img1_segs = self.get_seg(vid_name)
        img0_obj, img1_obj = self.get_obj(vid_name)

        # img0 = np.resize(img0, (512, 512, 3))
        # img1 = np.resize(img1, (512, 512, 3))
        # img0_segs = np.resize(img0_segs, (512, 512))
        # img1_segs = np.resize(img1_segs, (512, 512))
        # img0_obj = np.resize(img0_obj, (4, 512, 512))
        # img1_obj = np.resize(img1_obj, (4, 512, 512))

        imgs = [img0.copy(), img1.copy()]
        imgs_segs = [img0_segs.copy(), img1_segs.copy()]
        imgs_obj = [img0_obj.copy(), img1_obj.copy()]

        if self.co_transform is not None:
            # In unsupervised learning, there is no need to change target with image
            imgs, imgs_segs, imgs_obj, _ = self.co_transform(imgs, imgs_segs, imgs_obj, {})

        if self.input_transform is not None:
            imgs, imgs_segs, imgs_obj = self.input_transform((imgs, imgs_segs, imgs_obj))

        data = {'img0': imgs[0], 'img0_segs': imgs_segs[0], 'img0_obj': imgs_obj[0],
                'img1': imgs[1], 'img1_segs': imgs_segs[1], 'img1_obj': imgs_obj[1],
                'video_path': video_path}

        if self.ap_transform is not None:
            img0_ap, img1_ap = self.ap_transform([imgs[0], imgs[1]])
        else:
            img0_ap, img1_ap = imgs[0], imgs[1]

        data['img0_ph'] = img0_ap
        data['img1_ph'] = img1_ap
...
if __name__ == '__main__':
    ...
    dataset_train = EchoDynamic(root=args.motion_path, split="train", **real_kwargs)
