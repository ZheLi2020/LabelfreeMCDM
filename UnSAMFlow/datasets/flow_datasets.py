...
class ImgSeqDataset(torch.utils.data.Dataset, metaclass=ABCMeta):
    def __init__(self, root, full_seg_root, key_obj_root=None, name="",
    ...
    def _load_sample(self, s):
        ...
        for p in s["imgs"]:
            ...
            image = (imageio.imread(os.path.join(self.root, p)).astype(np.float32)/ 255.0)
            imgs.append(image)

            full_seg = imageio.imread(os.path.join(self.full_seg_root, p))[:, :, None]
            full_segs.append(full_seg)

            if self.key_obj_root is not None:
                key_obj = (np.load(os.path.join(self.key_obj_root, p[:-4] + ".npy"))/ 255.0)
    ...
    def __getitem__(self, idx):
        ...
        if self.co_transform is not None:
            ...
            imgs, full_segs, key_objs, _ = self.co_transform(imgs, full_segs, key_objs, {})

        if self.input_transform is not None:
            imgs, full_segs, key_objs = self.input_transform((imgs, full_segs, key_objs))

        # adj_maps = full_segs_to_adj_maps(torch.stack(full_segs), win_size=9)

        data.update({"img1": imgs[0], "img2": imgs[1],
                     "full_seg1": full_segs[0], "full_seg2": full_segs[1],})

        # process key_objs to keep exactly three objects (to make sure the number of objects is fixed so that we can form batches)
        if self.key_obj_root is not None:
            place_holder = torch.full((1, *key_objs[0].shape[1:]), np.nan, dtype=torch.float32)

            if key_objs[0].shape[0] == 0:
                key_obj = place_holder
            else:
                valid_key_obj = (key_objs[0].mean(axis=(1, 2)) >= 0.005)  ## some objects may be too small after cropping
        ...
        if self.ap_transform is not None:
            data["img1_ph"], data["img2_ph"] = self.ap_transform([imgs[0].clone(), imgs[1].clone()])

        return data


class KITTIRawFile(ImgSeqDataset):
    def __init__(self, root, full_seg_root, key_obj_root, name="kitti-raw",
                 ap_transform=None, input_transform=None, co_transform=None,):
        super(KITTIRawFile, self).__init__(root, full_seg_root, key_obj_root, name,
...
class KITTIFlowMV(ImgSeqDataset):
    ...
    def __init__(self, root, full_seg_root, key_obj_root, name="", input_transform=None,
                 co_transform=None, ap_transform=None,):
        super(KITTIFlowMV, self).__init__(root, full_seg_root, key_obj_root, name,
                                          input_transform=input_transform,
                                          co_transform=co_transform, ap_transform=ap_transform,)
    ...
    def collect_samples(self):
        ...
        if self.test_mode:
            ...
            for i, sample in enumerate(samples):
                ...
                samples[i].update({"flow_occ": os.path.join("flow_occ", filename),
...
class SintelRaw(ImgSeqDataset):
    def __init__(self, root, full_seg_root, key_obj_root, name="", input_transform=None,
                 ap_transform=None, co_transform=None,):
        super(SintelRaw, self).__init__(root, full_seg_root, key_obj_root, name,
                                        input_transform=input_transform,
...
class Sintel(ImgSeqDataset):
    def __init__(self, root, full_seg_root, key_obj_root, name="", dataset_type="clean",
                 split="train", subsplit="trainval", with_flow=False, input_transform=None,
                 co_transform=None, ap_transform=None,):
        ...
        super(Sintel, self).__init__(root, full_seg_root, key_obj_root, name,
                                     input_transform=input_transform,
                                     co_transform=co_transform, ap_transform=ap_transform,)
    ...
    def __getitem__(self, idx):
        ...
        if self.with_flow:
            data["flow_gt"] = load_flow(self.samples[idx]["flow"]).astype(np.float32)
            data["occ_mask"] = (imageio.imread(self.samples[idx]["occ_mask"]).astype(np.float32)[:, :, None]/ 255.0)
    ...
    def collect_samples(self):
        ...
        with open(sp_file, "r") as f:
            for line in f.readlines():
                ...
                scene = path_split[-2]
                sample = {"imgs": ["/".join(img1.split("/")[-4:]),
                                   "/".join(img2.split("/")[-4:]),]}
                if self.with_flow:
                    sample["flow"] = os.path.join("/".join(path_split[:-3]), "flow",
                                                  scene, path_split[-1][:-4] + ".flo",)
                    sample["occ_mask"] = os.path.join("/".join(path_split[:-3]), "occlusions",
                                                      scene, path_split[-1],)
