...
from datasets.get_dataset import get_double_dataset
...
def main_single(cfg, args):
    ...
    # prepare data
    train_sets, valid_sets, train_sets_epoches = get_double_dataset(cfg.data)
    logging.info(f"Echo train sets: {train_sets.get_length()} samples")
    logging.info(f"Echo val sets: {valid_sets.get_length()} samples")

    train_loader = torch.utils.data.DataLoader(train_sets, batch_size=1,
                                               num_workers=cfg.train.workers, pin_memory=True)

    # prepare validation dataset
    valid_loader = torch.utils.data.DataLoader(valid_sets, batch_size=1, num_workers=4, pin_memory=True,
    ...
    trainer = get_trainer(cfg.trainer)(train_loader, valid_loader, model, loss, cfg.save_root, cfg.train, 'Echo',
                                       resume=True, train_sets_epoches=train_sets_epoches, device=args.device,
                                       n_gpu=args.n_gpu)

    trainer.validate_without_gt(cfg)
...
def main(args):
    ...
    run_dir = 'aug+hg+mf20250731-100335-solar-bird-54'
    args.name = os.path.basename(args.config)[:-5]
    cfg.save_root = os.path.join(args.exp_folder, run_dir)
    if not os.path.exists(cfg.save_root):
        os.makedirs(cfg.save_root, exist_ok=True)

    set_logger(log_level='info', fname=os.path.join(cfg.save_root, 'output_eval.log'))
