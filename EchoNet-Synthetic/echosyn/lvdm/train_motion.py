...
from echosyn.common import (instantiate_from_config, pad_reshape, padf, unpad_reshape, unpadf,)
...
def log_validation(config, unet, vae, scheduler, accelerator, weight_dtype, epoch, val_dataset):
    ...
    if config.unet._class_name == "UNetSpatioTemporalConditionModel":
        dummy_added_time_ids = torch.zeros(
            (len(config.validation_lvefs), config.unet.addition_time_embed_dim),
    ...
    with torch.no_grad(), torch.autocast("cuda"):
        ...
        B, C, T, H, W = (len(config.validation_lvefs), 4, config.validation_frames,
                         config.unet.sample_size, config.unet.sample_size,)
        latents = torch.randn((B, C, T, H, W), device=accelerator.device,
        ...
        if config.unet._class_name == "UNetSpatioTemporalConditionModel":
            dummy_added_time_ids = torch.zeros((B, config.unet.addition_time_embed_dim),
                                               device=accelerator.device, dtype=weight_dtype,)
            forward_kwargs["added_time_ids"] = dummy_added_time_ids

        # reverse diffusionn loop
        for t in timesteps:
            latent_model_input = (torch.cat([latents] * 2) if config.validation_guidance > 1.0
                                  else latents)
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            tracker.log({"validation": wandb.Video(videos, caption=("Lvefs: " + ", ".join([str(e) for e in config.validation_lvefs])),
                        fps=config.validation_fps,)})
...
def parse_args():
    ...
    parser.add_argument("--config", default='configs/default_uncondition.yaml', type=str, help="Path to the config file.")
    parser.add_argument("--no_wandb", action="store_true", help="Disable wandb logging.", default=False)
...
def main():
    ...
    # unet_trained = unet.__class__.from_pretrained(config.unet_pre_trained,)

    # Merge weights from the pre-trained model onto the new model for any layer that has the exact same parameters
    # Get the state dictionaries
    # trained_state_dict = unet_trained.state_dict()
    # model_state_dict = unet.state_dict()

    # Filter out keys where the shapes match
    # filtered_state_dict = {k: v for k, v in trained_state_dict.items()
    #                        if k in model_state_dict and v.shape == model_state_dict[k].shape}
    #
    # # Optionally, print out keys that are skipped (for debugging)
    # for k in trained_state_dict:
    #     if (k in model_state_dict and trained_state_dict[k].shape != model_state_dict[k].shape):
    #         print(f"Skipping key '{k}': pretrained shape {trained_state_dict[k].shape} vs model shape {model_state_dict[k].shape}")

    # Update the model's state dict and load it
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, shuffle=True, batch_size=config.train_batch_size,
    ...
    lr_scheduler = get_scheduler(config.lr_scheduler, optimizer=optimizer,
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            ...
            tracker_config,
            init_kwargs={"wandb": {"group": config.wandb_group,
    ...
    progress_bar = tqdm(range(0, config.max_train_steps), initial=initial_global_step, desc="Steps",
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # latent_motions = batch['motion'] # B x S x E
                # latent_motions = torch.randn(
                #     (latents.shape[0], 1, config.unet.cross_attention_dim),
                #     device=accelerator.device,
                #     dtype=weight_dtype,
                # )
                latent_motions = batch["motion"]  # B x 3072
                latent_motions = latent_motions[:, None, :]
                # S = Sequence_length => defined in the config as num_latent_motions
                # E = Embedding_dim =>  defined in the config as cross_attention_dim
                ...
                timesteps = torch.randint(0, int(noise_scheduler.config.num_train_timesteps),
                ...
                if config.get("input_perturbation", 0) > 0.0:
                    noisy_latents = noise_scheduler.add_noise(latents, noise + config.input_perturbation
                                                              * torch.rand(1,).item() * torch.randn_like(noise),
                                                              timesteps,)
                else:
                ...
                forward_kwargs = {"timestep": timesteps,
                                  ...
                                  "encoder_hidden_states": latent_motions,}
            ...
            if accelerator.sync_gradients:
                ...
                global_step += 1
                accelerator.log({"train_loss": train_loss, "prediction_mean": prediction_mean,
                                 "prediction_std": prediction_std, "target_mean": target_mean,
                                 "target_std": target_std, "mean_losses": mean_losses,},
                                step=global_step,)
                train_loss = 0.0
                prediction_mean = 0.0
                prediction_std = 0.0
