...
import sys
sys.path.append('/vol/idea_longterm/ot70igyn/EchoNet-Synthetic/')

from echosyn.common import (load_model, pad_reshape, padf, parse_formats, save_as_avi,
                            save_as_gif, save_as_img, save_as_mp4, unpad_reshape, unpadf,)
from echosyn.common.datasets import ImageSet, TensorMotionEF, TensorSet, TensorMotionEFSpecial
...
if __name__ == "__main__":
    ...
    parser.add_argument("--unet", type=str, default='../../experiments/lvdm_ef_motion_300/checkpoint-460000/unet_ema', help="Path unet checkpoint.")
    parser.add_argument("--vae", type=str, default='/vol/ideadata/at70emic/projects/EchoSynExt/models/ivae', help="Path vae checkpoint.")
    # parser.add_argument("--conditioning", type=str, default='/vol/ideadata/at70emic/projects/TMI23/samples/lidm_vpred/aprivacy/latents', help="Path to the folder containing the conditionning latents.",)
    parser.add_argument("--conditioning", type=str,
                        default='/vol/ideadata/at70emic/projects/EchoSynExt/data/latents/mean/dynamic/Latents',
                        help="Path to the folder containing the conditionning latents.", )
    parser.add_argument("--motion", type=str,
                        default='/vol/idea_longterm/ot70igyn/EMA-VFI/log/train_EchoNet/20250509-212414/real_train300',
                        help="Path to the folder containing the motion vectors.", )

    parser.add_argument("--output", type=str, default="../../experiments/lvdm_ef_motion_300/ef_motion_460000_128", help="Output directory.")
    parser.add_argument("--num_samples", type=int, default=2048, help="Number of samples to generate.")
    ...
    parser.add_argument("--save_as", type=parse_formats, default=['mp4'],
        help="Save formats separated by commas (e.g., avi,jpg). Available: avi, mp4, gif, jpg, png, pt",)
    parser.add_argument("--frames", type=int, default=128, help="Number of frames to generate. Must be a multiple of 32",)
    ...
    # 2 - Load models
    print('Load unet model from path ', args.unet)
    print('Save videos to path ', args.output)
    ...
    if file_ext == "pt" and args.motion is None:
        dataset = TensorSet(args.conditioning)
    elif file_ext == "pt" and args.motion is not None:
        dataset = TensorMotionEF(args.conditioning, args.motion, args.chunk)
        # dataset = TensorMotionEFSpecial(args.conditioning, args.motion, args.chunk)
    else:
    ...
    format_input = (pad_reshape if config.unet._class_name == "UNetSpatioTemporalConditionModel"
                    else padf)
    format_output = (unpad_reshape if config.unet._class_name == "UNetSpatioTemporalConditionModel"
                     else unpadf)

    B, C, T, H, W = (args.batch_size, config.unet.out_channels, config.unet.num_frames,
    ...
    with torch.no_grad():
        while not finished:
            for cond in dataloader:
                ...
                cond, motion, lvefs, latent_name, motion_name = cond
                ...
                # lvefs = torch.randint(args.min_lvef, args.max_lvef + 1, (B,), device=device,
                #                       dtype=dtype, generator=generator,)
                # lvefs = torch.tensor([args.max_lvef] * B, device=device, dtype=torch.float32)
                lvefs = lvefs / 100.0
                lvefs = lvefs[:, None, None].to(torch.float32)  # B x 1 x 1
                lvefs = lvefs.repeat_interleave(TR, dim=0)  # B*TR x 1 x 1
                lvefs = lvefs.to(device)
                # forward_kwargs["encoder_hidden_states"] = lvefs


                motion = motion.to(device, dtype)  # B x N
                motion = motion[:, None, :]  # B x 1 x N
                motion = motion.repeat_interleave(TR, dim=0)  # B*TR x 1 x N
                forward_kwargs["encoder_hidden_states"] = torch.cat([lvefs, motion], dim=2)
                ...
                with torch.autocast("cuda"):
                    for t in timesteps:
                        ...
                        # Stitching
                        inputs = torch.cat([latent_model_input[:, r * (T - OT) : r * (T - OT) + T]
                                            for r in range(TR)], dim=0,)  # B*TR x T x 2C x H+P x W+P
                ...
                for chunk in chunked_latents:
                    decoded_chunks.append(vae.decode(chunk.float().to(device)).sample.cpu())
                ...
                # save samples
                for j in range(B):
                    output_name = f"sample_{sample_index:06d}_{args.chunk}"
                    # FileName,EF,ESV,EDV,FrameHeight,FrameWidth,FPS,NumberOfFrames,Split
                    filelist.append([output_name, file_lvefs[j], 0, 0, video.shape[1], video.shape[2],
                                     fps, video.shape[0], "TRAIN", latent_name[j], motion_name[j]])
                    if "mp4" in args.save_as:
                        save_as_mp4(video[j], os.path.join(args.output, "mp4", f"{output_name}.mp4"),)
                    if "avi" in args.save_as:
                        save_as_avi(video[j], os.path.join(args.output, "avi", f"{output_name}.avi"),)
                    if "gif" in args.save_as:
                        save_as_gif(video[j], os.path.join(args.output, "gif", f"{output_name}.gif"),)
                    if "jpg" in args.save_as:
                        save_as_img(video[j], os.path.join(args.output, "jpg", f"{output_name}"), ext="jpg",)
                    if "png" in args.save_as:
                        save_as_img(video[j], os.path.join(args.output, "png", f"{output_name}"), ext="png",)
                    if "pt" in args.save_as:
                        torch.save(video[j].clone(), os.path.join(args.output, "pt", f"{output_name}.pt"),)
                    sample_index += 1
                    pbar.update(1)
    ...
    df = pd.DataFrame(filelist, columns=["FileName", "EF", "ESV", "EDV", "FrameHeight", "FrameWidth",
                                         "FPS", "NumberOfFrames", "Split", 'Latent_name', 'Motion_name'],)
    df.to_csv(os.path.join(args.output, f"FileList_{args.chunk}.csv"), index=False)
